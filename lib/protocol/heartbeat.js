...
module.exports = function(request) {
    ...
    this.log.debug("Handling heartbeat for agent %s.payload=%s", request.id, typeof request.payload !== 'undefined');
    this.log.trace("heartbeat payload is ",request);
    var defer = Q.defer();
    ...
    Q.nextTick(_.bind(function() {
        ...
        this.db.hget(request.id, "status", function(err, state) {
            if(err) defer.reject(err);
                ...
                // Update our status report data
                _this.statusReportData.agents[request.id] = {
                    id: request.id,
                    ts: moment.duration().humanize("LL")
                };
