                ...
                .exec(function(err) {
                    if(err) defer.reject(err);
                        ...
                        _this.ensureWatchDog(request).then(function(watchdog) {
                            ...
                            _this.executeOnAgent(request.id, 'configure', {restart: false}).then(function() {
                                _this.emit('agent-connected', request.id);
                            }, function(err) {
                                _this.log.error("Agent %s has thrown an error while configuring itself, %s", request.id, err);
                            });
