...
(function(){
    ...
    module.exports = function(request) {
        ...
        Q.nextTick(function() {
            ...
            if(!subscription) {
                subscription = new EventSubscription(_this, request);
            ...
            if(! _.contains(subscription.events, request.event)) {
                _this.on(request.event, subscription.listener);
     ...
     * @constructor
     */
    var EventSubscription = function(dispatcher, spec) {
        this.events = [];
        this.agentId = spec.id;

        this.listener = function(e) {
            // Append this event to our event list
            dispatcher.db.rpush(spec.id+"_events", JSON.stringify({key: spec.event, data:e}));
        };
