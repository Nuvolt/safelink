    ...
    Transport.prototype.send = function(sender, uuid, key, payload, options) {
        ...
        request({
            ...
            if(err || resp.statusCode >= 400)   {
                log.error("Error received from server for request, but no callback was provided. %s", err, JSON.stringify({url:sender.url, method:'POST', body:_.extend(payload || {}, {key:key, v: sender.version, id: sender.id}) }));
