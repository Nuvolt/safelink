...
var Dispatcher = (function()    {
    ...
    function Dispatcher(cfg) {
        ...
        this.statusReportTmpl = Handlebars.compile(fs.readFileSync(__dirname + '/status_report.hbs') + "");
        this.statusReportData = {agents:{}};
        ...
        server = http.createServer(function(req, res) {
            ...
            if(_this.suspended) {
                ...
                if (req.method === 'POST') {
                    ...
                    // Produce the safelink status report
                    _this.statusReportData.PENDING = _.filter(pendingCommands, function(c) { return c.status === 'PENDING'}).length;
                    _this.statusReportData.COMPLETE = _.filter(pendingCommands, function(c) { return c.status === 'COMPLETE'}).length;
                    _this.statusReportData.ACTIVE = _.filter(pendingCommands, function(c) { return c.status === 'ACTIVE'}).length;

                    res.end(_this.statusReportTmpl(_this.statusReportData));
                }
                else
        ...
        this.commandMonitorInterval = setInterval(function() {
            ...
            _this.statusReportData.GARBAGE_COLLECTED = 0;

            async.forEach(pendingCommands, function(cmd, callback) {
                ...
                if(cmd.status !== 'COMPLETE' && delta > _this.pendingCleanupThreshold) {
                    cmd.defer.reject(new Error("no-response"));
                    cmd.status = 'COMPLETE';
                    _this.statusReportData.GARBAGE_COLLECTED++;
    ...
    Dispatcher.prototype.executeOnAgent = function(agentId, commandKey, payload, options) {
        ...
        Q.nextTick(_.bind(function() {
            ...
            // If we have a socket, just send a command message right now
            if(this.agentSockets[agentId]) {
                this.log.debug("Sending command %s(%s) directly using established web socket channel", commandKey, cmdId);

                this.agentSockets[agentId].emit('command', {
                    id: cmdId,
                    key: commandKey,
                    agent: agentId,
                    ts: ts,
