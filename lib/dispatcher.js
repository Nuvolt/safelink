...
//                        res.end(JSON.stringify({success:false, v:VERSION, error:err}));
        ...
        this.on('agent-connected', function(e) {
            ...
            this.db.multi()
                ...
                .hset(e.id, "version", e.agent.version)
                .hset(e.id, "heartbeat-interval", e.meta.interval)
                .exec(function(err) {
                    if(err) defer.reject(err);
                    else {
                        _this.ensureWatchDog({id: e.id, interval: e.meta.interval}).then(function(watchdog) {
    ...
    Dispatcher.prototype.suspend = function(duration) {
        ...
        if(!this.suspended) {
            this.suspended = true;
            this.log.warn("Dispacther is now suspended");
            if(duration) {
                setTimeout(function() {
                    _this.resume();
                }, duration * 1000);
