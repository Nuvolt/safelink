...
var Dispatcher = (function()    {
    ...
    function Dispatcher(cfg) {
        ...
        this.statusReport = Handlebars.compile(fs.readFileSync(__dirname + '/status_report.hbs') + "");
        ...
        server = http.createServer(function(req, res) {
            ...
            if(_this.suspended) {
                ...
                if (req.method === 'POST') {
                    ...
                    var data = {
                        PENDING: _.filter(pendingCommands, function(c){ return c.status === 'PENDING'}).length,
                        COMPLETE: _.filter(pendingCommands, function(c){ return c.status === 'COMPLETE'}).length,
                        ACTIVE: _.filter(pendingCommands, function(c){ return c.status === 'ACTIVE'}).length
                    };
                    console.dir(data);

                    res.end(_this.statusReport(data));
                }
                else
