...
(function(){
    ...
    Agent = (function() {
        ...
        function Agent(cfg) {
            ...
            // A simple waiting command cache to track executing commands
        ...
        Agent.prototype.start = function() {
            ...
            Q.nextTick(function() {
                ...
                if(_this.websocket) {
                    _this.ws = new WebSocket(_this.websocket.dispatcher);
                    ...
                    _this.ws.on('message', function(msg) {
                        _this.log.warn("WS message received", msg);
                    });
                }
        ...
        Agent.prototype.subscribeTo = function(key, fn, options) {
            ...
            if(!handler || options.force) {
                ...
                dispatch(_this, 'subscribe', {
                    event:key,
                    options:options
                }, _.pick(options, "timeout"), function(err, resp) {
                    if(err)
        ...
        Agent.prototype.monitor = function(key, options) {
            return dispatchP('start-monitoring', {key:key, interval:options.interval || 5});
        };

        Agent.prototype.cancelMonitoring = function(key) {
            return dispatchP('stop-monitoring', {key:key});
        };
        ...
        Agent.prototype.execute = function(key, payload, options) {
            ...
            Q.nextTick(function() {
                ...
                // Keep a reference to our final deferred
                options.deferredExecution = defer;

                dispatch(_this, 'execute-command', {
                    commandKey:key,
                    payload:payload,
                    options:options||{}
                }, _.pick(options, "timeout"), function(err, resp, body){
                    if(err) {
        ...
        Agent.prototype.executeOn = function(agentId, key, payload, options) {
            ...
            Q.nextTick(function() {
                ...
                dispatch(_this, 'execute-command-on', {
                    ...
                    commandKey:key,
                    payload:payload || {},
                    options: _.omit(options||{}, "deferredExecution")
                }, _.pick(options, "timeout", "deferredExecution"), function(err, resp, body) {
                    if(err) {
                        defer.reject({success:false, error:err});
                    }
                    else if(resp && resp.statusCode >= 400) {
        ...
        function executeCommand(command) {
            ...
            if(handler) {
                ...
                Q.timeout(deferredResponse.promise, command.timeout * 1000).then(function(result) {
                    ...
                    dispatch(_this, "post-command-progress", {commandId: command.id, progress:progress}, {noResponse:true});
                });

            }
            else {
                this.log.warn("No handler configured for command %s(%s). We assume that a custom event handler has been installed on the 'command' event", command.key, command.id);
            }
        }
        ...
        function heartbeat() {
                    ...
                    if(_.isFunction(_this.heartbeatSpec.payload)) {
                        ...
                        if(_this.heartbeatSpec.payload.length == 1) {
                            ...
                            Q.timeout(defer.promise, (_this.heartbeatSpec.interval-5) * 1000 || 25000).then(function(data) {
                                ...
                                payloadDefer.resolve(dispatchP(_this, "heartbeat", {payload: data, meta: meta}));

                            }, function(err) {

                                // Send a late heartbeat without a payload
                                _this.log.warn(err, "Unable to produce heartbeat payload. Empty payload will be used");
                                payloadDefer.resolve(dispatchP(_this, "heartbeat", {meta: meta}));
                            });
                            ...
                            _this.log.trace(data, "Heartbeat payload produced");
                            return dispatchP(_this, "heartbeat", {payload: data, meta: meta});
                        }

                    }
                    else {
                        _this.log.trace(data, "Heartbeat payload produced");
                        return dispatchP(_this, "heartbeat", {payload: _this.heartbeatSpec.payload, meta:meta});
                    }

                }
                else
                    return dispatchP(_this, "heartbeat", {meta:meta});
            });
            ...
            result.then(function(resp) {
                ...
                _this.nextHeartbeat = setTimeout(_.bind(heartbeat, _this), _this.heartbeatSpec.interval * 1000);
            });
        ...
        function onCommandProgress(e) {
            ...
            var cmd = this.waitingCommands[e.cmd.id];
            if(cmd) {
                this.log.trace("command %s has been found and progress will be updated");
         ...
         * @param key
         * @param payload
         * @param options {Object} contains all options. timeout (default 30) is supported right now.
         * @returns {Promise} A promise for the command result.
         */
        function dispatchP(agent, key, payload, options) {
            var defer = Q.defer(), uuid = shortid.generate();
            ...
            if(options.deferredExecution) {
                var cmd = {
                    uuid: uuid,
                    defer: options.deferredExecution
                };
                agent.waitingCommands[uuid] = cmd;
            }

            request({
                url: agent.url,
                method:'POST',
                body: _.extend(payload, {uuid: uuid, key:key, v: agent.version, id: agent.id}),
                json:true,
                pool:false,
                timeout:options.timeout * 1000 || agent.timeout * 1000
            }, function(err, resp, body) {

                if(err || resp.statusCode >= 400)   {
                    agent.log.error("Error received from server for request, but no callback was provided. %s", err, JSON.stringify({url:agent.url, method:'POST', body:_.extend(payload, {key:key, v: agent.version, id: agent.id}) }));
         ...
         * @obsolete {dispatchP} should be used starting with version 0.9.0
         * @returns {*}
         */
        function dispatch(agent, key, payload, options, callback) {
            var uuid = shortid.generate();

            agent.log.debug("Executing command %s", uuid);
            ...
            return request({
                ...
                if(callback) {
                    ...
                    agent.log.trace("No callback, emitting a response event", body);
