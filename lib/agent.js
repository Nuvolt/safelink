    ...
    Agent = (function() {
        ...
        Agent.prototype.subscribeTo = function(key, fn, options) {
            ...
            if(!handler || options.force) {
                ...
                promise = Transports.send(this, 'subscribe',{
                    event:key,
                    options:options
        ...
        Agent.prototype.unsubscribeFrom = function(key) {
            ...
            if(handler) {
                ...
                promise = Transports.send(this, 'unsubscribe',  {
        ...
        Agent.prototype.broadcast = function(event, payload, options) {
            return Transports.send(this, 'broadcast',{ event:event, payload: payload || {}, options:options }, _.pick(options || {}, "timeout"));
        };
        ...
        Agent.prototype.monitor = function(key, options) {
            return Transports.send(this, 'start-monitoring', {key:key, interval:options.interval || 5});
        };

        Agent.prototype.cancelMonitoring = function(key) {
            return Transports.send(this, 'stop-monitoring', {key:key});
        };
        ...
        Agent.prototype.execute = function(key, payload, options) {
            return Transports.send(this, 'execute-command', { commandKey:key, payload:payload, options:options||{} }, _.pick(options || {}, "timeout"));
        };
        ...
        Agent.prototype.executeOn = function(agentId, key, payload, options) {
            return Transports.send(this, 'execute-command-on', { agentId: agentId, commandKey:key, payload:payload || {}, options: options || {} }, _.pick(options || {}, "timeout"));
        };
        ...
        Agent.prototype.emitTo = function(agentId, event, payload, options) {
            return Transports.send(this, 'emit-to', { agents: agentId, event:event, payload: payload, options:options }, _.pick(options, "timeout"));
        };
        ...
        Agent.prototype.retrieveCommands = function() {
            ...
            promise = Transports.send(this, 'retrieve-pending-commands');
        ...
        Agent.prototype.retrieveEvents = function(options) {
            ...
            promise = Transports.send(this, 'retrieve-pending-events');
            promise.then(function(result) {
        ...
        function executeCommand(command) {
            ...
            if(handler) {
                ...
                Q.timeout(deferredResponse.promise, command.timeout * 1000).then(function(result) {
                    ...
                    var promise = Transports.send(_this, 'command-response', {
                    ...
                    Transports.send(_this, 'command-error', {commandId: command.id, type: 'timeout', error: err});
                }, function(progress) {
                    _this.log.trace("Progress notification received for command %s(%s)", command.key, command.id);
                    Transports.send(_this, "post-command-progress", {commandId: command.id, progress:progress}, {noResponse:true})
                });
                    ...
                    if(_.isFunction(_this.heartbeatSpec.payload)) {
                        ...
                        if(_this.heartbeatSpec.payload.length == 1) {
                            ...
                            Q.timeout(defer.promise, (_this.heartbeatSpec.interval-5) * 1000 || 25000).then(function(data) {
                                ...
                                payloadDefer.resolve(Transports.send(_this, "heartbeat", {payload: data, meta: meta}));

                            }, function(err) {

                                // Send a late heartbeat without a payload
                                _this.log.warn(err, "Unable to produce heartbeat payload. Empty payload will be used");
                                payloadDefer.resolve(Transports.send(_this, "heartbeat", {meta: meta}));
                            });
                            ...
                            _this.log.trace(data, "Heartbeat payload produced");
                            return Transports.send(_this, "heartbeat", {payload: data, meta: meta});
                        }

                    }
                    else {
                        _this.log.trace(data, "Heartbeat payload produced");
                        return Transports.send(_this, "heartbeat", {payload: _this.heartbeatSpec.payload, meta:meta});
                    }

                }
                else
                    return Transports.send(_this, "heartbeat", {meta:meta});
            });
        ...
        function onCommandProgress(e) {
            ...
            var cmd = Transports.getResult(e.cmd.id);
            if(cmd) {
